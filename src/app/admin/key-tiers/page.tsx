...
export default function KeyTiersPage() {
  ...
  const [draft, setDraft] = useState<KeyTier | null>(null);
  ...
  const loadTiers = async () => {
    try {
      ...
      if (data.success) {
        ...
        setMessage(data.message || 'Failed to load key tiers');
      }
    } catch {
      setMessage('Failed to load key tiers');
    } finally {
      setLoading(false);
    }
  };
  ...
  const saveTiers = async (nextTiers: KeyTier[]) => {
    ...
    try {
      ...
      if (data.success) {
        setMessage('Key tiers saved successfully');
        await loadTiers();
        return true;
      }
      setMessage(data.message || 'Failed to save key tiers');
      return false;
    } catch {
      setMessage('Failed to save key tiers');
      return false;
    } finally {
      setSaving(false);
    }
  ...
  const initializeTiers = async () => {
    try {
      ...
      if (data.success) {
        await loadTiers();
        setMessage('Default key tiers initialized');
      } else {
  ...
  const openEdit = (absoluteIndex: number) => {
    ...
    setDraft({
      ...tiers[absoluteIndex],
      commissions: { ...tiers[absoluteIndex].commissions },
    });
  };
  ...
  const columns: DataTableColumn<KeyTier & { __index: number }>[] = [
      {
        key: 'name',
        header: 'Tier',
        render: (row) => (
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        icon={Layers}
        title="Key tiers"
        description="Commission rates by activation key price band"
        actions={
          <div className="flex flex-wrap gap-2">
            {tiers.length === 0 ? (
              <button
      ...
      <StatStrip
        items={[
          ...
          { label: 'Active', value: tiers.filter((t) => t.isActive).length },
          {
            label: 'How it works',
            value: 'Price → tier',
            hint: 'Key price selects commission band',
          },
        ]}
      />
      ...
      <AdminModal
        open={!!draft}
        title={editIndex === null ? 'Add key tier' : 'Edit key tier'}
        onClose={() => {
          setDraft(null);
          setEditIndex(null);
        }}
        ...
        {draft ? (
          <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label className="admin-label">Min key price (₹)</label>
                <input
                  type="number"
                  min={0}
                  className="admin-input"
              ...
              <div>
                <label className="admin-label">Max key price (₹)</label>
                <input
                  type="number"
                  min={0}
                  className="admin-input"
            ...
            <div>
              <p className="admin-label">Commission rates (flat ₹)</p>
